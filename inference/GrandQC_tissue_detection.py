...
def get_slide_names():
    ...
    return sorted([f for f in os.listdir(SLIDE_DIR) 
...
def get_patch_crop_coordinates(w, h, width, height, wi_n, he_n):
    ...
    if w != wi_n and h != he_n:
        ...
        return (w * PATCH_SIZE, h * PATCH_SIZE, 
                (w + 1) * PATCH_SIZE, (h + 1) * PATCH_SIZE)
    elif w == wi_n and h != he_n:
        # Right edge patch
        return (width - PATCH_SIZE, h * PATCH_SIZE, 
                width, (h + 1) * PATCH_SIZE)
    elif w != wi_n and h == he_n:
        # Bottom edge patch
        return (w * PATCH_SIZE, height - PATCH_SIZE, 
                (w + 1) * PATCH_SIZE, height)
    else:
...
def concatenate_patches_horizontally(w, wi_n, overhang_wi, mask, class_mask, 
                                   temp_image, temp_image_class_map):
    ...
    if w == 0:
        ...
        return (np.concatenate((temp_image, mask_cropped), axis=1),
                np.concatenate((temp_image_class_map, class_mask_cropped), axis=1))
    else:
        return (np.concatenate((temp_image, mask), axis=1),
                np.concatenate((temp_image_class_map, class_mask), axis=1))


def concatenate_rows_vertically(h, he_n, overhang_he, temp_image, temp_image_class_map,
                              end_image, end_image_class_map):
    ...
    if h == 0:
        ...
        return (np.concatenate((end_image, temp_image_cropped), axis=0),
                np.concatenate((end_image_class_map, temp_class_cropped), axis=0))
    else:
        return (np.concatenate((end_image, temp_image), axis=0),
...
def process_image_patches(image, model, preprocessing_fn):
    ...
    for h in range(he_n + 1):
        ...
        for w in range(wi_n + 1):
            ...
            mask, class_mask = process_single_patch(patch_image, model, preprocessing_fn)
        ...
        end_image, end_image_class_map = concatenate_rows_vertically(
            h, he_n, overhang_he, temp_image, temp_image_class_map,
            end_image, end_image_class_map
        )
...
def process_single_slide(slide_path: str, model_weights_path: str):
    ...
    mpp = round(float(slide_reader.convert_resolution_units(input_res=0, input_unit='level', output_unit='mpp')[0]), 4)
